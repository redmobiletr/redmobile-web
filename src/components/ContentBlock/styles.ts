...
export const HeroAnimatedHighlight = styled("div")<{ isVisible: boolean; isAnimating: boolean }>`
  position: relative;
  min-height: 60px;
...
export const HeroAnimatedText = styled("div")<{ isVisible: boolean; isAnimating: boolean }>`
  position: absolute;
  top: 0;
  left: 0;
  width: 100%;
  opacity: ${({ isVisible }) => (isVisible ? 1 : 0)};
  transform: translateY(${({ isVisible }) => (isVisible ? "0px" : "10px")});
